...
(function() {
    ...
    var basecamp_tt = {
        ...
        addAjaxLoaderToTasks: () => {
            basecamp_tt.getOptionStorage().then((optionStorage) => {
        ...
        onClickAddButtons: () => {
            ...
            addButtons.forEach((addButton) => {
                addButton.onclick = () => {
                    ...
                    basecamp_tt.getTaskStorage().then((taskStorage) => {
                        ...
                        basecamp_tt.getOptionStorage().then((optionStorage) => {
                            if(optionStorage.url && optionStorage.username && optionStorage.password) {
                                ...
                                fetch(optionStorage.url + "basecamp-extension-api?projectIds=" + includedProjects + "&maxHours=" + maxHours + "&overviewName=" + overviewName + "&showInOverview=" + showInOverview, {
                                    ...
                                    if(!response.ok) {
                                        basecamp_tt.addErrorMessage("Basecamp Time Tracker API Options - Error: " + response.statusText);
                                    } else {
                                ...
                                basecamp_tt.addErrorMessage("Basecamp Time Tracker API options not set!");
                            }
                        });
                    } else {
                ...
                var observer = new MutationObserver((mutations) => {
                    mutations.forEach((mutation) => {
                        if(mutation.target.classList.contains("completed_items_todo_list") && mutation.addedNodes.length === 2) {
                            ...
                            basecamp_tt.getTimeEntries("itemId", 3, mutation.target.firstChild);
